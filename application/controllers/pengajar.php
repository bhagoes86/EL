    ...
    function filter($segment_3 = '')
    {
        ...
        if (!empty($filter)) {
            $retrieve_all = $this->pengajar_model->retrieve_all_filter(
                $filter['nip'], $filter['nama'], $filter['nohp'], $filter['jenis_kelamin'], $filter['tempat_lahir'], $filter['tgl_lahir'], $filter['bln_lahir'], $filter['thn_lahir'], $filter['alamat'], $filter['status_id'], $filter['username'], $filter['is_admin'], $page_no
            );
        }
